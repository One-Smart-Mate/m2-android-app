...
class FirebaseStorageRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun uploadLogFile(
            userId: String,
            appVersion: String,
            fileUri: Uri,
        ): String =
            try {
                val fileName = fileUri.lastPathSegment ?: "logs_${System.currentTimeMillis()}.zip"
                val logReference =
                    firebaseStorage.reference.child("app/Android/logs/$userId/$appVersion/$fileName")

                logReference.putFile(fileUri).await()
                val url = logReference.downloadUrl.await()
                url.toString()
            } catch (e: Exception) {
                LoggerHelperManager.logException(e)
                FirebaseCrashlytics.getInstance().recordException(e)
