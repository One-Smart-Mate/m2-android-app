...
constructor(
    ...
    fun logCreateRemoteCardRequest(card: CreateCardRequest) {
        try {
            ...
            firebaseAnalytics.logEvent(
                "create_remote_card_request_event",
                bundleOf("card" to json)
            )
        } catch (e: Exception) {
            Log.e("test", "Error analytics ${e.localizedMessage}")
            FirebaseCrashlytics.getInstance().recordException(e)
    ...
    fun logCreateCardException(e: Throwable) {
        try {
            ...
            firebaseAnalytics.logEvent(
                "create_card_exception_message_event",
                bundleOf("value" to e.localizedMessage.orEmpty())
            )
        } catch (e: Exception) {
            Log.e("test", "Error analytics ${e.localizedMessage}")
            FirebaseCrashlytics.getInstance().recordException(e)
    ...
    fun logSyncCardException(e: Exception) {
        try {
            ...
            firebaseAnalytics.logEvent(
                "upload_card_exception_message_event",
                bundleOf("value" to e.localizedMessage.orEmpty())
            )
        } catch (e: Exception) {
            Log.e("test", "Error analytics ${e.localizedMessage}")
